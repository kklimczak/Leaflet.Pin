;(function () {
    L.Handler.MarkerPin = L.Handler.extend({
        ...
        _findClosestCircle: function (map, guideList, latlng) {
            var closest = L.GeometryUtil.closestLayerSnap(
                map,
                guideList,
                latlng
            )
        ...
        _parse: function (layer) {
            ...
            if (layer instanceof L.FeatureGroup) {
                ...
                if (
                    ...
                    circle._mRadius = JSON.parse(
                        JSON.stringify(layer.getRadius())
                    )
            ...
            if (found) {
                for (var i = 0; i < this._guideList.length; i++) {
                    if (this._guideList[i]._leaflet_id == id) {
                        if (latlng.length) {
                            this._guideList[i].setLatLngs(
                                L.LatLngUtil.cloneLatLngs(latlng)
                            )
                        } else {
                            this._guideList[i].setLatLng(
                                L.LatLngUtil.cloneLatLng(latlng)
                            )
                        }
                    }
                }
            ...
            if (this._markers) {
                ...
                if (e) {
                    ...
                    if (this._poly) {
                        ...
                        this._poly._latlngs[polyPointsAmount - 1] =
                            e.target._latlng
                        this._poly.redraw()
                    }
                }
    ...
    L.Edit.Marker.Pin = {
        ...
        _pin_on_dragstart: function (e) {
            if (!this._marker._pinning) {
                this._marker._pinning = new L.Handler.MarkerPin(
                    this._marker._map
                )
            }
